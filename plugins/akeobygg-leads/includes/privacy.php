...
add_filter(
	...
	function ( $exporters ) {
		$exporters['akeobygg-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'akeobygg-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'akeobygg-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'akeobygg-leads',
						'group_label' => __( 'Quote requests', 'akeobygg-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['akeobygg-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'akeobygg-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
