...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'akeobygg-leads' ),
					'singular_name' => __( 'Lead', 'akeobygg-leads' ),
					'edit_item'     => __( 'Förfrågan', 'akeobygg-leads' ),
					'search_items'  => __( 'Search leads', 'akeobygg-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'akeobygg-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'akeobygg-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'akeobygg-leads' ),
			'npl_phone'    => __( 'Phone', 'akeobygg-leads' ),
			'npl_service'  => __( 'Needs', 'akeobygg-leads' ),
			'npl_timeline' => __( 'When', 'akeobygg-leads' ),
			'npl_zip'      => __( 'ZIP', 'akeobygg-leads' ),
			'npl_source'   => __( 'Source', 'akeobygg-leads' ),
			'npl_status'   => __( 'Status', 'akeobygg-leads' ),
			'date'         => __( 'Received', 'akeobygg-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'akeobygg-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'akeobygg-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'akeobygg-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'akeobygg-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'akeobygg-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'akeobygg-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'akeobygg-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'akeobygg-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'akeobygg-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'akeobygg-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'akeobygg-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'akeobygg-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
