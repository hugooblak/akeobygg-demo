<?php
/**
 * AKEO Bygg AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "AKEO Bygg AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through ak_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'akeobygg', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'akeobygg' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'akeobygg' ) ) );

		register_block_pattern_category( 'akeobygg-sections', array( 'label' => __( 'AKEO Bygg AB sections', 'akeobygg' ) ) );
		register_block_pattern_category( 'akeobygg-pages', array( 'label' => __( 'AKEO Bygg AB page layouts', 'akeobygg' ) ) );
	}
);
