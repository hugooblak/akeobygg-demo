 ...
 * or photo. They call ak_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional AKEO Bygg AB.
 *
 ...
 * @return string
 */
function ak_sample_note() {
	return ak_lead( 'ar_riktig' ) ? '' : ' <span class="ak-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ak_lead_img().
 */
function ak_lead_bild( $i ) {
	$bilder = (array) ak_lead( 'bilder', array() );
 ...
 * @return string
 */
function ak_ring_label() {
	$namn = ak_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ak_lead( 'telefon' ) ) : 'Ring ' . ak_lead( 'telefon' );
}
